...
class Pipeline:
    ...
    def _run_contacts_pipeline(self) -> None:
        # TODO: implement — transition between states, call modules, handle errors
        pass
```
...
## 6. Struggle tracker (Modules 1-9 in progress)
...
## 7. Where to continue: Module 9 — Pipeline orchestrator (IN PROGRESS)

Module 8 (NotionWriter) is COMPLETE — all 4 tests green. Module 9 is IN PROGRESS.

### What's been done:
- `Pipeline.__init__` is implemented — creates all module instances
- `_transition()` is implemented — one-liner that updates `self.ctx.state`
- `_run_contacts_pipeline()` needs implementation
- Student was confused about two things (addressed below)

### Two issues identified and explained:
1. **Passed PipelineContext objects to _transition instead of PipelineState values.** Student wrote `self._transition(PipelineContext(state=PipelineState.DISCOVER_CONTACTS))` — should be `self._transition(PipelineState.DISCOVER_CONTACTS)`. The transition method updates the existing context's state, it doesn't create a new context.

2. **Only listed state transitions without calling the actual modules.** The method needs to both transition state AND call the module (e.g., `self.contact_discoverer.discover(...)`).

### Guiding questions already provided:
1. What arguments does `self.contact_discoverer.discover()` need, and where do they come from? (Answer: `self.ctx.target_job` and `self.ctx.resume` — pre-set on the context)
2. What Python construct wraps code that might fail so it doesn't crash the program? (Answer: try/except)

### Tests for Module 9 (test_pipeline.py) — already provided:
1. `test_pipeline_initializes_all_modules` — All module instances exist on the pipeline (1 PASSING)
2. `test_pipeline_starts_in_idle_state` — Context starts in IDLE (1 PASSING)
3. `test_pipeline_transitions_through_states` — State transitions occur during run (FAILING)
4. `test_pipeline_error_handling` — Errors caught, state → ERROR, error stored (FAILING)

### What _run_contacts_pipeline needs to do:
```
try:
    transition to DISCOVER_CONTACTS
        contacts = self.contact_discoverer.discover(self.ctx.target_job, self.ctx.resume)
        store contacts on self.ctx.contacts
    transition to GENERATE_MESSAGES
        self.message_generator.generate(self.ctx.contacts, self.ctx.target_job, self.ctx.resume)
    transition to WRITE_NOTION
        write job and contacts to Notion via self.notion_writer
    transition to COMPLETE
except Exception as e:
    transition to ERROR
    append error message to self.ctx.errors
```

### Key concepts to teach:
- State machine pattern — transitions bookend module execution
- Composition — Pipeline holds module instances, doesn't inherit from them
- try/except for error handling and graceful failure
- Storing results on context (`self.ctx.contacts`, `self.ctx.jobs`)
- CLI entry point with `sys.argv` parsing (after tests pass)

---

## 8. Remaining after Module 9

Nothing — Module 9 is the final module. After it's complete:
- Generate Module 9 quiz
- Generate final cumulative quiz covering all concepts
- Produce final struggle tracker showing growth arc from Module 1 to 9
- Add CLI entry point with `sys.argv` for `search`, `full`, and direct URL modes
...
> "I'm continuing a TDD build session. The transfer document above has all context. Module 9 (Pipeline orchestrator) is in progress. I have __init__ and _transition done. I need help implementing _run_contacts_pipeline. The two issues I hit were: passing PipelineContext objects to _transition instead of PipelineState values, and listing transitions without calling modules. Help me implement it using the same Socratic TDD approach."

The model should:
1. Acknowledge the context and the two issues already explained
2. Ask guiding questions about try/except and storing results on context
3. Guide through implementing _run_contacts_pipeline
4. After green: generate Module 9 quiz + final cumulative summary
5. Update the struggle tracker with final growth arc
